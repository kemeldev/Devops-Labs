...
Result showed *:80, meaning Apache was listening correctly on all interfaces. This ruled out Apache config as the cause.
	1. Check which firewalld zone is actually active on the network interface:
bash
...
This returned nothing — the key finding. It meant ens192 wasn't actually bound to any firewalld zone at runtime, even though firewall rules had been added to the public zone. Rules in a zone only apply to interfaces assigned to that zone.
	1. Confirmed the firewall rules themselves were correctly configured:
bash
   sudo firewall-cmd --list-all --zone=public
The rich rules allowing TCP port 80 from the two expected subnets were present and correctly formatted.
	1. Confirmed source IP was within the allowed range:
bash
   ip addr show
Verified 172.31.17.243/22 fell inside the 172.31.16.0/22 range already permitted by the rich rule — so the rule's source range wasn't the problem.
	1. Isolated firewalld as the cause by temporarily stopping it:
bash
