...
- Trigger several rolling updates and control their behaviour.
...
```bash
FIELD=host    # change to match what you found in §0.3: host, hostname, or pod

kubectl run tmp --rm -it --image=nicolaka/netshoot:latest --restart=Never -- sh -c "
  for i in \$(seq 1 60); do
    curl -s -m 3 http://node-api/api/info
    echo
  done
...
kubectl run tmp --rm -it --image=nicolaka/netshoot:latest --restart=Never -- sh -c "
  for i in \$(seq 1 20); do curl -s -m 3 http://node-api/api/info; echo; done
" 2>/dev/null | python3 -c "
import sys, json, collections
c = collections.Counter()
for line in sys.stdin:
...
```bash
for i in $(seq 1 20); do curl -sS -m 3 http://localhost:30080/api/node/api/info; echo; done \
| python3 -c "
import sys, json, collections
c=collections.Counter()
for line in sys.stdin:
...
> 2What state are the Pods in? How many of each?
