...
No new VM — KinD runs Kubernetes' control-plane and worker components as Docker containers on top of the existing Docker Engine. Check free resources first (`free -h`); the plan assumes Lab 02's Compose stack is stopped while this lab is in progress, and the KinD cluster itself now runs 3 node-containers (1 control-plane + 2 workers) instead of 1, which costs a bit more RAM than the original single-worker plan — worth confirming there's headroom before starting.
...
|---|---|---|
| a `services:` entry | Deployment | manages a set of replica Pods |
