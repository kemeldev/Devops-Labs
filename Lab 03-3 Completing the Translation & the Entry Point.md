...
In Lab 02, `nginx.pathbased.conf` stripped the path prefix with a `rewrite` directive. With ingress-nginx, you'd have used `nginx.ingress.kubernetes.io/rewrite-target: /$2` plus a regex capture group.

Traefik does not understand that annotation. Not "does it differently" — **does not understand it at all**, and won't tell you. Path stripping in Traefik is a `Middleware` object, attached to the router by a differently-named annotation.

**This is the annotation-portability lesson, made concrete.** You are not editing a string to migrate controllers. You are introducing a **custom resource** that only exists because Traefik installed it, and rewriting the annotation that references it. Then, in Step 7, you'll express the same rewrite in **six lines of typed, validated, portable Gateway API spec** with no annotation and no CRD.
...
Routing is fine; the browser is calling the wrong URL.
Open DevTools → Network, look at the failing request's URL. If it has an IP or a port in it, the bundle was built with absolute URLs — go back to §2.1 and rebuild. **No amount of routing configuration fixes this**, because the URL is compiled into the JavaScript.
