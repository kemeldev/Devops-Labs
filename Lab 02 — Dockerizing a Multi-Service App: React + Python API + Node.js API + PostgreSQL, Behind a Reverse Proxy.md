...
Step 0 — Prep & Docker fundamentals refresher
	...
	• Here basically check we are running docker , in case is and older version we can uninstall it and install a newer version
		docker version
...
Step 8 — Reverse proxy: two variants (mirrors Lab 01 Step 3)
	...
	• ( for this LAB 02 doing Varian B does is not that important or relevant, we can leave this as a future exercise)
...
When you containerize a React app, you usually pick a Docker base image like:
But you should not randomly pick node:20-alpine. You first check the project files to see if the app requires a specific Node.js version.
...
In this step, we create a Dockerfile for the React frontend.
...
We use a multi-stage Docker build:
	...
	1. Build stage
		a. Uses Node.js to install dependencies and build the React/Vite app.
	2. Runtime stage
		a. Uses Nginx to serve the already-built static files.



One design change from the original draft, worth calling out: the frontend container does not need its own nginx location /api/ proxy rule. 
Once edge-proxy is in front of everything (Step 8), the browser only ever talks to the proxy's address — for / and for /api/py, /api/node. The react-frontend container just serves static files; it never needs to know the APIs exist.
...
WORKDIR /app
COPY package*.json ./
RUN npm ci  (npm ci is better for Docker builds because it installs the exact dependency versions from package-lock.json.)
COPY . .
...
These lines mean: If no value is provided during docker build, use /api/py and /api/node as the default API paths.
...
Or this command:  docker run --rm react-frontend:0.1.0 sh -c "grep -rho '/api/py' /usr/share/nginx/html/assets/*.js | head -1"

This checks whether the value /api/py was successfully written into the built JavaScript files.

Sh : This starts a shell inside the container.
The -c option means: Run the following text as a shell command.  sh -c "some command here"
|  The pipe takes the output from the command on the left and sends it to the command on the right.
	Search for /api/py, then send the results to head -1.
...
#(requirements.txt is usually created by the developer, not automatically by Python )
A developer installs packages, then runs:

pip freeze > requirements.txt
This writes installed package versions into requirements.txt.

#These are the external Python packages this app needs in order to run.”

fastapi==0.118.0 (This is the web API framework.)
uvicorn[standard]==0.37.0 (This is the web server that runs the FastAPI app.) (FastAPI defines the application, but something has to actually listen on a port and serve HTTP traffic.)
psycopg[binary]==3.2.10 (This is the PostgreSQL driver.)
python-dotenv==1.1.1 (This package loads environment variables from a .env file.)


python3 --version   # just for reference, doesn't matter what the host has
grep -i "python_requires\|python-version" * -r 2>/dev/null

Some Python packages install easily:
fastapi
python-dotenv
They are mostly Python code.
But some packages talk to system libraries or contain optimized C/Rust code. PostgreSQL drivers are a common example.
psycopg2
psycopg2-binary
psycopg[binary]
Asyncpg

These packages may need extra tools when being installed, such as:
gcc
make
...
Those tools are called compiler tooling or build dependencies.
...
EOF
4. Write the Dockerfile
Same multi-stage discipline as the frontend, adapted for Python: build into a virtual environment in the build stage, 
copy only the finished venv into a slim runtime stage, 
run as a non-root user, and healthcheck against the app's existing /health endpoint using Python itself rather than installing curl (one less package in the final image, smaller attack surface — a small thing, but it's the kind of habit worth building now).
Dockerfile
...
#gcc This is a C compiler. Some Python packages include C code and need a compiler during installation.
#libpq-dev  This provides PostgreSQL development libraries and headers.
#--no-install-recommends (install only the required packages, not extra recommended ones.)
#rm -rf /var/lib/apt/lists/* This removes package list cache files after installation.
RUN python -m venv /opt/venv
#This creates a Python virtual environment inside the image
ENV PATH="/opt/venv/bin:$PATH"
# This modifies the PATH.  
So if the existing PATH is something like: /usr/local/bin:/usr/bin:/bin
Then it becomes :  /opt/venv/bin:/usr/local/bin:/usr/bin:/bin
The : separates directories in PATH.

Why doing this?,  Because you created a virtual environment here: RUN python -m venv /opt/venv
...
a system group named app
a system user named app
puts the user app into the group app
Why? By default, containers often run as root unless told otherwise.
So this Dockerfile creates a dedicated non-root user to run the API.
...
The build context is the folder Docker uses when building the image. 
If you are inside: ~/multi-service-App/api-python, then docker will use this folder /home/kemel/multi-service-App/api-python

6. Test it standalone against a throwaway Postgres — isolate the DB variable before Compose
Create a temporary network and Postgres container matching the repo's expected credentials:

The -e means: Set an environment variable inside the container.
...
Both APIs should reach Postgres:
...
This runs a Python command inside the python-api container. 
...
Both should print an IP address — DNS resolves because python-api/node-api and postgres-db share db-net.

172.21.0.2 is the internal Docker network IP of the postgres-db container on db-net. It uses Docker’s private internal network.
...
That error pattern (empty response, unhealthy) means the app never came up and started listening — not a DB or network issue, since node user is correct and the container process itself is the problem. Let's see why before touching Compose.
Run these and paste back the output:
bash
docker ps -a --filter name=test-node-api
docker logs test-node-api --tail 50

And check what the app's actual entry point is — I had you use CMD ["node", "index.js"] as a guess, and if the real file is server.js or app.js, that alone would cause exactly this symptom (container exits immediately, nothing ever binds to 8002, curl gets nothing, healthcheck never passes):
